...
- Status: expanded matrix; E09.6 repository route contract added for Cloud UI HAProxy flows; live
  production PKI/mTLS evidence remains pending.
...
| HAProxy -> frontend | TLS 1.2 if backend TLS is enabled; backend TLS mode: `internal_http` by default | optional; `backend_mtls` is explicit deployment choice | internal/corporate PKI if TLS enabled | frontend service DNS/name or container network identity | HAProxy backend ACL and Kolla service placement | platform owner | backend connection with untrusted cert rejected if TLS selected | E09.6 | E09.6 repository route contract for placeholder `cloud-ui.example.invalid`; live scan pending | backend TLS/mTLS and network ACL proof pending |
| HAProxy -> API | TLS 1.2 if backend TLS is enabled; backend TLS mode: `internal_http` by default | optional; `backend_mtls` is explicit deployment choice | internal/corporate PKI if TLS enabled | API service DNS/name or container network identity | HAProxy backend ACL, trusted proxy headers and `/api/v1/health/ready` health check path | platform owner | backend connection with untrusted cert rejected if TLS selected | E09.6 | E09.6 repository route contract for placeholder `cloud-ui.example.invalid`; live scan pending | backend TLS/mTLS and trusted proxy evidence pending |
