...
Целевая система — расширяемый административный портал для OpenStack Epoxy 2025.1, развернутого через Kolla Build и Kolla-Ansible на Rocky Linux. Портал должен быстро отображать большие объемы данных по виртуальным машинам и гипервизорам, поддерживать собственные группы ресурсов и прикладные роли, запускать разрешенные многошаговые операции через Mistral, интегрироваться с Watcher и Masakari, предоставлять аудит и безопасно расширяться под новые API.
