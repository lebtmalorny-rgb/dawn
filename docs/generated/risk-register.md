...
| R-071 | All-in-one role-managed lab deploy mistaken for full E09 acceptance | The 2026-06-28 lab run now includes a bounded Kolla-Ansible-side AIO role path: preflight passed, `cloud-ui-aio-reconfigure.yml` converged four digest-pinned containers on one all-in-one node, idempotency passed with migration disabled, and API/UI smoke plus container hardening were confirmed. It still does not prove upstream Kolla `site.yml`/tag integration, three-node/twelve-container topology, HAProxy/VIP/TLS, SELinux labels, corporate registry signing/scanning or failed-update rollback acceptance. | Keep `docs/generated/e09-deployment-smoke-evidence.md` scoped to `partial_lab_role_reconfigured_all_in_one` and require separate sanitized evidence for each remaining gate before acceptance. | E09 |
