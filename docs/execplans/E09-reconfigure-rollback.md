...
- [x] 2026-06-25: Контракт и тестовый double. Evidence:
  `backend/.venv/bin/python -m pytest tests/test_e09_reconfigure_rollback.py -q` failed `8 failed`
  because lifecycle defaults, lifecycle task include, validation assertions, role test updates and
  generated evidence are not implemented yet.
- [x] 2026-06-25: Минимальная реализация. Evidence:
  `backend/.venv/bin/python -m pytest tests/test_e09_reconfigure_rollback.py -q` passed `8 passed`
  after adding lifecycle defaults, lifecycle facts task, main include, validation and evidence docs.
- [x] 2026-06-25: Отрицательные сценарии и безопасность. Evidence: E09.7 tests verify dry-run-only
  status, no live Kolla commands in lifecycle tasks, data-preserving disable/uninstall and no
...
- [x] 2026-06-25: Интеграционные и пользовательские проверки. Evidence:
  ...
  passed `39 passed`; `backend/.venv/bin/python -m pytest tests -q` passed `51 passed`;
  `make test` passed backend `327 passed, 1 skipped` and frontend `35 passed`.
- [x] 2026-06-25: Документация, evidence и review. Evidence: E09.7 generated evidence,
  DKB traceability and risk register updated; `make lint`, `make typecheck`, `make security` and
...
- 2026-06-25: Older E09 tests over-specified cross-slice implementation details: E09.6 asserted the
  exact global `cloud_ui_config_version`, and E09.4 asserted the full task include list. They were
  relaxed to assert stable HAProxy and migration-order contracts while allowing the E09.7 lifecycle
  include.
...
E09.7 repository lifecycle contract is implemented and verified locally. It provides a dry-run-only,
data-preserving deployment lifecycle interface and evidence document, but it is not live
`kolla-ansible` reconfigure, rolling update, failed rollback, disable/uninstall or E09 acceptance
evidence. Registry digest pulls, scanner/signature/SBOM tied to deployed images, test inventory run,
container inspection, HAProxy/TLS smoke, management ACL proof and ДКБ-69 waiver remain external gates
for E09.8/test-stand execution.
