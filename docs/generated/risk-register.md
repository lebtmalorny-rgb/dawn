...
| R-063 | E09.3 provisioning contract mistaken for live DB/RabbitMQ proof | E09.3 adds `deploy/kolla/ansible/roles/cloud_ui_provisioning` and sanitized blocker evidence. The approved test host first reported `vault_package_unavailable`; after official HashiCorp RPM repository approval, the endpoint returned HTTP `404` with `x-amzn-waf-reason: geo`. Live Vault, MariaDB and RabbitMQ provisioning did not run. | Provide a reachable approved Vault/SecMan package source, approved mirror or pre-installed approved lab Vault, then rerun E09.3 live provisioning and least-privilege checks before claiming DB/MQ evidence. | E09 |
