...
| R-071 | All-in-one manual lab deploy mistaken for full E09 acceptance | The 2026-06-28 lab run deployed digest-pinned Cloud UI images on one all-in-one node, ran migration, confirmed DB/RabbitMQ readiness, container hardening and API/UI smoke. It does not prove the live Kolla-Ansible role path, three-node/twelve-container topology, HAProxy/VIP/TLS, SELinux labels, corporate registry signing/scanning or failed-update rollback acceptance. | Keep `docs/generated/e09-deployment-smoke-evidence.md` scoped to `partial_lab_deployed_all_in_one` and require separate sanitized evidence for each remaining gate before acceptance. | E09 |
