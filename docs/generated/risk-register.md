...
| R-063 | E09.3 provisioning contract mistaken for live DB/RabbitMQ proof | E09.3 adds `deploy/kolla/ansible/roles/cloud_ui_provisioning` and sanitized blocker evidence. The approved test host currently reports `vault_package_unavailable`, so live Vault, MariaDB and RabbitMQ provisioning did not run. | Provide an approved Vault/SecMan package source or pre-installed approved lab Vault, then rerun E09.3 live provisioning and least-privilege checks before claiming DB/MQ evidence. | E09 |
