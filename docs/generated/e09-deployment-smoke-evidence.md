...
- Rollback snapshot: `/root/cloud-ui-aio-rollback-20260628T112402Z` on the test host; contents are not committed because runtime env/inspect files can contain secrets
...
| rollback | partial_lab_evidence | Rollback pending for full E09 acceptance. The Kolla CLI path run created rollback snapshot `/root/cloud-ui-aio-rollback-20260628T112402Z`; snapshot files are retained only on the test host because inspect/env backups can contain secrets. |
| live reconfigure | passed_aio_kolla_cli_path | `playbooks/cloud-ui-aio-reconfigure.yml` was synced to the Ansible host and executed through `kolla-ansible reconfigure -p ... -t cloud-ui`. Preflight completed with `ok=10 changed=0 failed=0`; the idempotent reconfigure with migration disabled completed with `ok=34 changed=0 failed=0 skipped=2`. This is a bounded AIO Kolla CLI custom-playbook path, not full upstream `site.yml` service integration or three-node acceptance. |
...
| Kolla CLI AIO reconfigure | `run-cloud-ui-aio-kolla.py reconfigure-no-migration` invoked `kolla-ansible reconfigure -p /etc/kolla/cloud-ui-sync-bundle/playbooks/cloud-ui-aio-reconfigure.yml -t cloud-ui`; recap `openstack-aio : ok=34 changed=0 failed=0 skipped=2`. |
...
- ДКБ-82: rollback snapshot, AIO role reconfigure evidence, Kolla CLI custom-playbook path evidence
  and smoke evidence exist for the test all-in-one deployment. Full upstream Kolla `site.yml` service
  integration, three-node Kolla-Ansible reconfigure/upgrade and failed-rollback acceptance remain
  pending.
