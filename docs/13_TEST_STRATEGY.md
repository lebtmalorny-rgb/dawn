...
- lost Mistral response lookup without duplicate execution;
- optional all-in-one Mistral smoke skipped by default and read-only unless explicit safe test config is provided;
...
- integration;
- `make test-integration` when an explicit live-smoke configuration exists; otherwise optional live
  tests must skip and report the skipped state;
- E2E;
