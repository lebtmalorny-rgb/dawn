...
- generation: 1
- elapsed seconds: 3.212341
- peak Python memory MiB: 3.625

## Read-model scenarios

| Scenario | Resource | Page size | Rows returned | p95 seconds | SQL p95 | SQL max | SQLite EXPLAIN summary |
|---|---|---:|---:|---:|---:|---:|---|
| `instances_default_page` | instances | 50 | 50 | 0.006665 | 5 | 5 | SEARCH instances USING INDEX ix_instances_project_status (cloud_id=? AND region_id=? AND deleted_at=?); USE TEMP B-TREE FOR ORDER BY |
| `instances_filtered_project_status` | instances | 50 | 50 | 0.003186 | 5 | 5 | SEARCH instances USING INDEX ix_instances_project_status (cloud_id=? AND region_id=? AND deleted_at=? AND project_id=? AND status=?) |
| `hypervisors_default_page` | hypervisors | 50 | 50 | 0.004892 | 5 | 5 | SEARCH hypervisors USING INDEX ix_hypervisors_service (cloud_id=? AND region_id=? AND deleted_at=?); USE TEMP B-TREE FOR ORDER BY |
| `hypervisors_filtered_service_status_az` | hypervisors | 50 | 50 | 0.003752 | 5 | 5 | SEARCH hypervisors USING INDEX ix_hypervisors_az (cloud_id=? AND region_id=? AND availability_zone=?) |
