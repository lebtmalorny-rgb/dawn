...
- 2026-06-24: Enforce `CLOUD_UI_SOURCE_PIN` in the build wrapper by creating temporary backend and
  frontend source directories from `git archive`, then adding a separately hashed prebuilt frontend
  ...
  verified explicitly.
- 2026-06-24: Reject `KOLLA_BUILD_CONFIG` and `KOLLA_DOCKER_DIR` overrides in the E09.1 wrapper and
  validate the rendered config contains the sanitized backend/frontend source paths and resolved
  commit. Reason: custom config or template directories can bypass reviewed source/template
  substitution, so this slice uses checked-in Kolla files until a later deployment slice defines a
  reviewed extension mechanism.
